...
describe('parseLatexResume', () => {
  it('parses output generated by compileLatexResume back into a valid ResumeSpec', () => {
    const originalSpec: ResumeSpec = {
      profile: {
        name: 'Jordan Belfort',
        ...
        location: 'New York, NY',
        links: {
          github: 'https://github.com/jordanb',
          linkedin: 'https://linkedin.com/in/jordanb',
        },
      },
      summary: 'Systems architect specialized in low latency distributed execution.',
      experiences: [
        {
          company: 'Stratton Oakmont Tech',
          role: 'Lead Systems Architect',
          location: 'New York, NY',
          startDate: '2021',
          endDate: 'Present',
          bullets: [
            { text: 'Engineered order matching engine handling 100k msgs/sec (ev-012).' },
          ],
        },
      ],
      education: [
    ...
    const compiledTex = compileLatexResume(originalSpec);
    const parsed = parseLatexResume(compiledTex);

    expect(parsed).not.toBeNull();
    const validated = ResumeSpecSchema.safeParse(parsed);
    expect(validated.success).toBe(true);

    expect(parsed!.profile.name).toBe('Jordan Belfort');
    expect(parsed!.profile.email).toBe('jordan@example.com');
    expect(parsed!.profile.links.github).toBe('https://github.com/jordanb');
    expect(parsed!.experiences).toHaveLength(1);
    expect(parsed!.experiences[0].company).toBe('Stratton Oakmont Tech');
    expect(parsed!.experiences[0].bullets[0].text).toContain('order matching engine');
    expect(parsed!.skills).toHaveLength(1);
    expect(parsed!.skills[0].category).toBe('Languages');
    expect(parsed!.skills[0].skills).toEqual(['C++', 'Rust', 'Go']);
    expect(parsed!.education).toHaveLength(1);
    expect(parsed!.education[0].institution).toBe('NYU');
  });

  it('parses standard Overleaf / Jake-style \\resumeSubheading LaTeX templates', () => {
    const jakeTex = `
\\documentclass[letterpaper,11pt]{article}
\\begin{document}
\\begin{center}
    \\textbf{\\Huge \\scshape Jane Doe} \\\\ \\vspace{1pt}
    \\small 555-555-5555 $|$ \\href{mailto:jane@example.com}{jane@example.com} $|$ 
...
\\section{Experience}
  \\resumeSubHeadingListStart
    \\resumeSubheading
      ...
      {Acme Cloud Inc}{San Francisco, CA}
      \\resumeItemListStart
        \\resumeItem{Architected multi-region failover protocol reducing downtime by 99.9% (ev-043).}
        \\resumeItem{Spearheaded Kubernetes migration across 40 microservices.}
      \\resumeItemListEnd
...
\\section{Education}
  \\resumeSubHeadingListStart
    \\resumeSubheading
      ...
      {B.S. in Electrical Engineering and Computer Science}{2016 -- 2020}
  \\resumeSubHeadingListEnd

\\section{Technical Skills}
 \\begin{itemize}[leftmargin=0.15in, label={}]
    \\small{\\item{
     \\textbf{Languages}{: TypeScript, Go, Python, SQL} \\\\
     \\textbf{Cloud and DevOps}{: AWS, Docker, Kubernetes, Terraform}
    }}
 \\end{itemize}
\\end{document}
`;

    const parsed = parseLatexResume(jakeTex);
    expect(parsed).not.toBeNull();
    ...
    expect(parsed!.profile.phone).toBe('555-555-5555');
    expect(parsed!.profile.links.github).toBe('https://github.com/janedoe');
    expect(parsed!.profile.links.linkedin).toBe('https://linkedin.com/in/janedoe');

    expect(parsed!.experiences).toHaveLength(1);
    expect(parsed!.experiences[0].company).toBe('Acme Cloud Inc');
    expect(parsed!.experiences[0].role).toBe('Senior Software Engineer');
    expect(parsed!.experiences[0].startDate).toBe('Jan 2021');
    expect(parsed!.experiences[0].endDate).toBe('Present');
    expect(parsed!.experiences[0].bullets).toHaveLength(2);
    expect(parsed!.experiences[0].bullets[0].citations).toContain('ev-043');

    expect(parsed!.education).toHaveLength(1);
    expect(parsed!.education[0].institution).toBe('Stanford University');
    expect(parsed!.education[0].degree).toBe('B.S. in Electrical Engineering and Computer Science');
    expect(parsed!.education[0].year).toBe('2020');

    expect(parsed!.skills).toHaveLength(2);
    expect(parsed!.skills[0].category).toBe('Languages');
    expect(parsed!.skills[0].skills).toEqual(['TypeScript', 'Go', 'Python', 'SQL']);
    expect(parsed!.skills[1].category).toBe('Cloud and DevOps');
  });
});
